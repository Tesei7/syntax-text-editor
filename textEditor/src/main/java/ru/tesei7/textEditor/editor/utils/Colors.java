package ru.tesei7.textEditor.editor.utils;

import java.awt.Color;

public interface Colors {
	Color DEFAULT_TEXT = Color.BLACK;
	Color KEY_WORD = new Color(0, 0, 128);
	Color COMMENT = new Color(128, 128, 128);
	Color IDENTIFIER = new Color(120, 75, 75);
	Color STRING_LITERAL = new Color(0, 128, 0);
	Color ANNOTATION = new Color(128, 128, 0);
	Color NUMBER_LITERAL = new Color(0, 0, 255);

	Color CURRENT_LINE_BACKGROUND = new Color(255, 250, 227);
	Color SELECTED_BACKGROUND = new Color(89, 116, 171);
	Color FOUND_BRACKET_BACKGROUND = new Color(153, 204, 255);
	Color NOT_FOUND_BRACKET_BACKGROUND = new Color(255, 220, 220);
}
